...
cat("  mycols(.01, .0, .01)  ", '\n')
cat("  p<-ggplot(tmp, aes(x=소비분야, y=freq, fill=Gender)) +  ", '\n')
...
cat("     labs(title='성별과 소비분야', x='소비분야', y='빈도수')  ", '\n')
cat("    ", '\n')
cat("  p + theme(  ", '\n')
...
cat("     axis.title.x = element_text(size = 12, color = 'black'), # X축 제목 스타일  ", '\n')
cat("     axis.title.y = element_text(size = 12, color = 'black') # Y축 제목 스타일  ", '\n')
cat("    )  ", '\n')
cat("   ", '\n')
invisible(readline(prompt="Press [enter] to continue"))
