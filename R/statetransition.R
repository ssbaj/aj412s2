...
statetransition <- function(Adata){
	...
	cat("  \033[1;31m* 주의:  상태를 나타내는 코드값들 중 0이 있으면 에러가 발생합니다 \033[0m",'\n')
	cat("  \033[1;31m* 주의:  상태 코드값들은 반드시 1 이상으로 기록하세요 \033[0m",'\n')
	...
	cat("    df2<-df[,c(2:3)] ",'\n')
	cat("  \033[1;31m* 변수는 모두 numeric으로 지정해야 에러가 없음 \033[0m ",'\n')
        cat("    index<-c(1, 2)  ",'\n')
        cat("    df2<-lapply(df2[ , index], as.numeric )  ",'\n')
