e_data<-function(explaining=0){

if(explaining==0) {
cat("\014")
cat("\033[1;34m**************************************************************** \033[0m ", '\n')
cat("\033[1;31m## 벡터 -------\033[0m ", '\n')
cat("  x<-c(1:10, 14, 21:24)", '\n')
cat("  x<-rep(8, 5)", '\n')
cat("  x<-rep(1:3, 2)", '\n')
cat("   ", '\n')
cat("\033[1;31m## 행렬 -------\033[0m ", '\n')
cat("  x<-matrix(1:9, nrow=3, byrow=T) ", '\n')
cat("  x[1,3]", '\n')
cat("   ", '\n')
cat("\033[1;31m## 데이터프레임 -------\033[0m ", '\n')
cat("  df<-data.frame(  ", '\n')
cat("     id=c(1,2,3,4),  ", '\n')
cat("     gender=c('남','여','여','남','남','여','남','여'),  ", '\n')
cat("     expend=c(800, 1400, 2000, 1600, 1400, 1600, 1340, 1900),  ", '\n')
cat("     income=c(2000, 2300, 4500, 4000, 3000, 3500, 2800, 4500)  )  ", '\n')
cat("  df[ ,-c(1,3)]  ", '\n')
cat("  df[df$gender=='남', ]  ", '\n')
cat("  df[df$gender=='남', c(2, 3)]  ", '\n')
cat("  df[df$id>=3, c(2, 3)]  ", '\n')
cat("   ", '\n')
cat("\033[1;31m## 리스트 -------\033[0m ", '\n')
cat("  x<-rep(1:3, 3)  ", '\n')
cat("  df<-data.frame(  ", '\n')
cat("     id=c(1,2,3,4),  ", '\n')
cat("     gender=c('1','1','2','2'),  ", '\n')
cat("     income=c(2000, 2300, 4500, 4000)  )  ", '\n')
cat("  datalist<-list(x, df)  ", '\n')
cat("  datalist[[1]]  ", '\n')
cat("  datalist[[2]]$income  ", '\n')
cat("   ", '\n')
cat("\033[1;31m## 팩터 ------- \033[0m ", '\n')
cat("  범주형변수는 factor로 지정한다. 단, 숫자의 경우 0을 ", '\n')
cat("  factor로 변경하고 다시 숫자numeric으로 바꾸면 0은 1이 된다 ", '\n')
cat("  area<-c('경기', '인천', '서울') ", '\n')
cat("  x<-as.factor(area) ", '\n')
cat("  df<-data.frame(  ", '\n')
cat("     id=c(1,2,3,4),  ", '\n')
cat("     gender=c('1','1','2','2'),  ", '\n')
cat("     income=c(2000, 2300, 4500, 4000)  )  ", '\n')
cat("  df$gender2<-factor(df$gender, levels=c(1, 2), labels=c('남', '여'))  ", '\n')
cat("   ", '\n')
cat("\033[1;31m## 0을 팩터로 지정하면... ------- \033[0m ", '\n')
cat("  x<-c(0,1,2) ", '\n')
cat("  y<-as.factor(x) ", '\n')
cat("  as.numeric(y) ", '\n')
cat("  [1] 1 2 3 ", '\n')
cat("   ", '\n')
cat("\033[1;31m## 변수 설명을 추가하는 방법... ------- \033[0m ", '\n')
cat("  library(Hmisc) ", '\n')
cat("  label(df$id) <- '피조사자 ID' ", '\n')
cat("  label(df$gender) <- '성별' ", '\n')
cat("  label(df$income) <- '소득' ", '\n')
} }

