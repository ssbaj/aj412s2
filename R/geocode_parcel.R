geocode_parcel<-function(vworld_apikey, addr){
  ...
  if (base::missing(vworld_apikey)) {
    ...
    cat(" \033[1;34m# addr <- '경기도 수원시 영통구 원천동 산19' \033[0m ", '\n')
    ...
    cat(" \033[1;32m df<-openxlsx(DataSet.xlsx) # 주소의 변수명: addr \033[0m ", '\n')
    ...
    cat(" \033[1;32m longlat<-geocode_parcel(my_vworld, addr)  \033[0m ", '\n')
